...
- `python scripts/verify_native_installer_lane.py --cutover-evidence-file <path>` 用于注入 approvals / rollback / clean-machine validation evidence；deliverables 归档时会转写为 `native-installer-cutover-evidence.json`。
