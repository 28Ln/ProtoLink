...
- 当前 extension loading boundary 只到 descriptor registry，不到运行时动态加载
...
- descriptor registry 是当前正式的 extension loading boundary；它只说明“可被识别和列举”，不说明“已被导入和激活”。
...
2. 签名与时间戳受控发布流程
3. 从 descriptor registry 推进到 controlled loading boundary
4. HIL / 长稳回归规划
...
- 当前交付不是原生签名安装器正式发布线
- 脚本能力不是不受信沙箱
- 插件扩展当前只开放到 manifest discovery / validation / audit / descriptor registry / preflight gate，不含 controlled runtime loading、动态加载或正式 SDK
