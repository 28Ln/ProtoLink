...
- `verify_native_installer_lane.py` 默认是 readiness probe，并会输出 `current_canonical_release_lane`、`native_installer_lane_phase`、`blocking_items`、`next_action`。
- `verify-native-installer-scaffold` 现在会校验 native installer scaffold 的 lifecycle / identity contract；`verify_native_installer_lane.py` 也会把 `lifecycle_contract_ready` 纳入 stage truth。
...
- 这些命令只在 signed native installer cutover 评估时启用，不是当前 bundled baseline 的默认 blocker。
