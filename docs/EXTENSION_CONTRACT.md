...
当前已经实现的是“显式、受控、Class A-only”的最小 runtime loading boundary。

正确的下一步不是开放任意动态加载，而是：
1. 保持 manifest / descriptor / loading plan 的单一事实源
2. 把 Class A runtime loading 补齐到日志证据与 failure evidence
3. 设计 Class B review workflow 的工程落点
4. 继续默认阻断 Class C runtime activation
5. 把 runtime state 继续纳入 headless summary、发布门禁与交付口径
...
2. release-preflight / headless-summary 必须能解释 runtime load state，而不是只停留在 loading plan
