...
manifest 必须先通过静态校验，才允许进入后续发现/装载流程。当前版本已做到 discovery / validation / audit / descriptor listing / loading plan，并提供显式 Class A runtime loading CLI；仍不开放自动激活或任意动态加载。

## 5.1 Descriptor registry, loading plan, and controlled Class A runtime boundary
...
6. `uv run protolink --plan-extension-loading` 基于 `registry.json` 生成装载计划
7. `uv run protolink --load-enabled-extensions` 作为显式 Class A runtime loading 入口
8. `uv run protolink --release-preflight` 复用同一条 Class A runtime gate 作为正式交付门禁

这条边界当前的工程含义是：

- ProtoLink 已承认哪些插件候选存在、通过静态审计并被 `registry.json` 显式启用
- 只有 enabled 且 `effective_state=eligible_for_loading` 的 Class A 扩展，才会被显式导入并执行 `register()`
- Class B 当前仍停留在 `review_required`
- Class C 当前即使进入 `high_risk_enabled` 计划状态，也不会被该 runtime CLI 自动执行

当前版本仍然不承诺：

- 应用启动时自动加载外部扩展
- Class B / Class C runtime activation
- owner surface / UI / transport / automation 注入
- 不受控的第三方动态加载
- 把 `list-extension-descriptors`、`plan-extension-loading` 或 `load-enabled-extensions` 误解为“正式 SDK 已开放”

若要继续跨过这条边界，必须进入下一阶段的 runtime governance，而不是直接开放任意动态加载。

## 5.2 Current runtime register contract

当前 `--load-enabled-extensions` 只支持最小 runtime boundary：

- entrypoint 必须采用 `module:function` 形式
- `register()` 允许 0 个或 1 个位置参数
- 若为 1 个参数，则传入受控 `ExtensionLoadContext`
- `register()` 返回值必须为 mapping 或 `None`
- CLI 输出 `runtime_load_report`，包含 `attempted_count` / `loaded_count` / `failed_count` / `skipped_count`
- `release-preflight` 会复用同一 runtime gate，并在 `load_failed` 时写入 failure evidence
- 非 ready 情况下 CLI 返回非零退出码
...
当前已经实现的是“显式、受控、Class A-only”的最小 runtime loading boundary，并已接入 release-preflight 与 failure evidence。

正确的下一步不是开放任意动态加载，而是：
1. 保持 manifest / descriptor / loading plan 的单一事实源
2. 设计 Class B review workflow 的工程落点
3. 继续默认阻断 Class C runtime activation
4. 把 runtime state 继续纳入更清晰的 headless summary 与生命周期模型
5. 明确 `loaded / started / stopped / failed` 状态治理
6. 更后面才谈正式 SDK，而不是直接开放第三方动态导入

## 14. Next boundary after explicit Class A runtime loading

当前之后的下一条正式边界应当是 runtime governance，而不是更开放的动态加载，至少要求：

1. Class A runtime load 失败要留下结构化日志与 failure evidence
2. headless-summary 必须在不隐式执行插件代码的前提下，更清楚解释 runtime load state
3. Class B 需要显式 review / approval 机制
4. Class C 继续默认阻断，不因 `allow_high_risk_plugins=true` 就自动进入 runtime execution
5. 生命周期 `loaded / started / stopped / failed` 需要正式状态模型
6. 所有扩展仍必须遵守 workspace first、evidence first、UI 不承担 I/O

当前正式口径应为：

- 已支持 descriptor registry
- 已支持 loading plan
- 已支持显式 Class A runtime loading
- 未开放 Class B / Class C runtime activation
- 更未开放任意第三方动态加载
