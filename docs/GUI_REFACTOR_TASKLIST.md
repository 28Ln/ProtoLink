...
- `uv run python scripts/run_full_test_suite.py --json-only` -> `332 passed`
...
| GUI-102 | P1 | In Progress | 收敛滚动层级，避免多层滚动并存 | 主窗口/面板滚动策略统一 |
...
| GUI-106 | P2 | Pending | 完成跨模块截图巡检与视觉一致性复核 | 截图验收矩阵、consistency 结论 |
