# ProtoLink Current State

Last updated: 2026-04-16

## 当前真实进展

ProtoLink 已完成正式基线收敛、第一次交付瘦身、运行证据补强，并进入 native installer 路线的可执行构建阶段。

当前状态分为三层：

- 已完成：`PL-012` Delivery Baseline Consolidation
- 已完成：`PL-013` Package Slimming and Delivery Hardening
- 进行中：`PL-014` Native Installer and Signing Path

### 已完成
- Windows-first 桌面工程骨架已经建立
- 统一工作区、设置、日志、导出、打包链路已经建立
- 传输能力已覆盖：Serial / TCP Client / TCP Server / UDP / MQTT Client / MQTT Server
- 协议工作流已覆盖：Modbus RTU / Modbus TCP 的基础调试闭环
- 自动化能力已覆盖：自动应答、规则引擎、脚本控制台、定时任务、通道桥接
- 回归体系、release-staging 验证、fresh-install 验证已经可执行
- 正式文档、任务台账、风险台账、handoff 入口已经冻结为单一口径
- portable/distribution/installer package 已完成第一轮 package slimming
- session service 的 shutdown / close 失败已经进入统一 failure evidence，并进入 release-preflight 阻断
- `verify_dist_install.py` 已支持多版本 dist 产物选择策略
- 已新增 WiX/MSI scaffold 构建与校验命令
- 已新增 native installer toolchain 检测命令
- 已新增 MSI build 与签名校验命令
- 已新增 native installer scaffold manifest 的 lifecycle contract 字段与校验，覆盖 `target_arch`、`install_scope`、`install_dir_name`、`product_code_policy`、`upgrade_strategy` 与静默安装命令
- 已新增 native installer scaffold included-entry checksums，覆盖 `ProtoLink.wxs`、`ProtoLink.Generated.wxi` 与 payload digest 校验
- 已新增 native installer lane JSON receipt 落盘能力，以及 `dist/deliverables` 顶层 deliverables manifest
- 已新增 `verify_release_deliverables.py`，可复核 deliverables manifest、archive checksums、package verifier 结果与 native lane receipt
- 已新增 `docs/NATIVE_INSTALLER_CUTOVER_POLICY.json` 作为签名 / 时间戳 / 审批 / 回滚要求的机器可读真值
- 已新增 native installer lane `policy_status`，可按 `signing` / `timestamp` / `approvals` / `rollback` / `clean_machine_validation` 输出分节 blocker
- 已新增 machine-readable cutover evidence 输入源与 `native-installer-cutover-evidence.json` 归档
- 已新增 `workspace/plugins/*/manifest.json` 的发现、静态校验、审计报告与 release-preflight 阻断
- 已新增 valid manifest 到 extension descriptor registry 的受控列举边界
- 已新增基于 `registry.json` 的 controlled extension loading plan 生成能力
- 已新增 `uv run protolink --load-enabled-extensions` 显式 Class A runtime loading 入口，只执行 enabled 且 `effective_state=eligible_for_loading` 的扩展 `register()`
- `uv run protolink --release-preflight` 已把 enabled Class A runtime load 结果纳入正式门禁，并在 `load_failed` 时写入结构化 failure evidence
- Class B 当前仍为 `review_required`，Class C 即使进入 `high_risk_enabled` 计划状态，也不会被该入口自动执行

### 当前可交付能力
- `uv run protolink --headless-summary`
- `uv run protolink --smoke-check`
- `uv run protolink --release-preflight`
- `uv run protolink --audit-plugin-manifests`
- `uv run protolink --list-extension-descriptors`
- `uv run protolink --plan-extension-loading`
- `uv run protolink --load-enabled-extensions`
- `uv run protolink --build-installer-package <name>`
- `uv run protolink --verify-installer-package <archive>`
- `uv run protolink --install-installer-package <archive> <staging> <install>`
- `uv run protolink --uninstall-portable-package <install-dir>`
- `uv run protolink --build-native-installer-scaffold <name>`
- `uv run protolink --verify-native-installer-scaffold <scaffold-dir>`
- `uv run protolink --verify-native-installer-toolchain`
- `uv run protolink --build-native-installer-msi <scaffold-dir>`
- `uv run protolink --verify-native-installer-signature <msi-file>`

## 当前验证基线

- `uv run python scripts/run_full_test_suite.py` -> `369 passed`
- `uv run python scripts/verify_canonical_truth.py --expected-mainline PL-014 --expected-pytest-count 369` -> passed
- `uv run python scripts/run_targeted_regressions.py --suite all` -> passed
- `uv run python scripts/audit_gui_layout.py --output-dir dist\gui-audit\latest` -> passed
- `uv run python scripts/verify_release_staging.py --name ci` -> passed
- `uv run python scripts/verify_release_staging.py --name local` -> passed
- `python scripts/verify_dist_install.py --artifact-version 0.2.5` -> passed
- `python scripts/run_soak_validation.py --cycles 2 --sleep-ms 0 --require-all-ready` -> passed
- `python scripts/verify_native_installer_lane.py` -> passed
- `uv run python scripts/build_release_deliverables.py --name release-0.2.5 --target-dir dist\deliverables` -> passed
- `uv run python scripts/verify_release_deliverables.py --target-dir dist\deliverables` -> passed
- `uv build` -> passed

## 当前 UI 工程判断

- 主窗口已从“说明区与底部 dock 挤压主工作面”的失衡结构，重构为以主工作面为中心的分栏布局
- 报文分析台已改为 tab + splitter 结构，默认 dock 高度已收敛
- Modbus RTU / Modbus TCP / 寄存器监视 / 自动化规则已完成第一轮 tab 化
- Serial / MQTT / TCP / UDP 面板已完成第二轮 tab 化与状态区换行
- Hero、左侧导航、右侧说明区与底部报文分析台已完成第三轮减重与首屏聚焦
- `dist/deliverables/` 已可稳定产出 release / portable / distribution / installer 归档、`deliverables-manifest.json`、`native-installer-lane-receipt.json`、`native-installer-cutover-evidence.json` 并完成安装自检
- GUI 收口任务、剩余视觉问题与交付级验收标准已沉淀为 `docs/GUI_REFACTOR_TASKLIST.md`
- dashboard、owner surfaces、Modbus 与传输面板的产品化文案已完成进一步收口
- `audit_gui_layout.py` 在目标分辨率下已达到 `highest_severity=clean`，dashboard 首屏与报文分析台的可见压缩警告已收敛
- 仓库根目录与 dist 下的临时 probe / smoke / 过期 0.2.4 dist 产物已完成清理

## 当前工程判断

ProtoLink 现在已经具备：
- 可运行
- 可回归
- 可打包
- 可安装
- 可交接
- 可为原生安装器路线生成与校验 WiX/MSI scaffold
- 可校验 native installer scaffold 的 lifecycle / identity contract，而不只是 payload checksum
- 可执行 MSI build / signature verify CLI（取决于本机 toolchain）
- 可把 native installer lane probe truth 与 deliverables 级证据一起落盘归档
- 可独立校验 `dist/deliverables` 中的 manifest、receipt 与归档一致性
- 可对 `workspace/plugins` 下插件 manifest 做静态发现、校验与审计，并将 invalid manifest 进入预检阻断
- 可列出当前工作区通过静态校验的 extension descriptors，并生成基于 `registry.json` 的 controlled loading plan
- 可通过显式 CLI 对 enabled 且 `eligible_for_loading` 的 Class A 扩展执行 `register()`
- 已具备 2.0 收尾文档基线：`PROJECT_FLOW_2_0` / `FEATURES_2_0` / `ISSUE_REGISTER_2_0` / `DEPENDENCY_AUDIT_2_0` / `HANDOFF_2_0`

ProtoLink 当前还不具备：
- 原生签名 Windows 安装器正式发布线
- 面向不受信环境的脚本沙箱
- Class B / Class C 扩展的 runtime activation、自动激活或不受控动态加载
- 面向扩展的正式 SDK 与完整生命周期治理
- 硬件在环（HIL）级别的长期回归体系

## 当前工作重点

当前重点已经从“交付基线收敛”和“第一次交付瘦身”推进到：

- 把当前 WiX scaffold / toolchain / MSI build / signature verify CLI 推进到受控发布 lane
- 把 soak 验证脚本推进到更长时间、更高负载的验证策略
- 保持 0.2.5 现有 bundled-runtime 交付链稳定可回退
- 在已完成 Class A runtime evidence / preflight gate 的基础上，继续推进 lifecycle model、Class B review workflow 与 Class C 长期阻断策略
