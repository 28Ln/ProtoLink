...
- 已新增基于 registry.json 的 controlled extension loading plan 生成能力（仅规划，不执行动态加载）
...
- 可列出当前工作区通过静态校验的 extension descriptors，作为 `PL-015` 的 controlled loading boundary 前置边界

ProtoLink 当前还不具备：
- 原生签名 Windows 安装器正式发布线
- 面向不受信环境的脚本沙箱
- 面向插件/协议扩展的 controlled runtime loading、动态加载与正式 SDK 契约
- 硬件在环（HIL）级别的长期回归体系
...
- 把 descriptor registry 推进到 controlled loading boundary，但不提前承诺任意第三方动态加载
