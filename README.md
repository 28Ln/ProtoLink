...
- `uv run protolink --list-extension-descriptors` 会列出当前工作区通过静态校验的扩展 descriptor registry；它定义了当前正式的 extension loading boundary，但不等于动态加载已开放。
...
- `workspace/plugins/<plugin-id>/manifest.json` 的发现、静态校验、descriptor registry 列举与 release-preflight 阻断
