...
class TcpClientProfile:
    format_version: str = "protolink-tcp-client-v1"
    selected_preset_name: str | None = None
    draft: TcpClientDraft = field(default_factory=TcpClientDraft)
    presets: list[TcpClientPreset] = field(default_factory=list)


def default_tcp_client_profile_path(profiles_dir: Path) -> Path:
    return profiles_dir / "tcp_client.json"


def load_tcp_client_profile(path: Path) -> TcpClientProfile:
    return load_transport_profile(
        path,
        format_version="protolink-tcp-client-v1",
        default_values={
            "host": "127.0.0.1",
            "port": 502,
            "send_mode": "hex",
        ...
        build_draft=lambda values, selected: TcpClientDraft(
        ...
        build_preset=lambda name, values: TcpClientPreset(
            name=name,
            host=str(values.get("host", "127.0.0.1")),
            port=int(values.get("port", 502)),
            send_mode=str(values.get("send_mode", "hex")),
        ...
        build_profile=lambda version, selected, draft, presets: TcpClientProfile(
...
def save_tcp_client_profile(path: Path, profile: TcpClientProfile) -> None:
