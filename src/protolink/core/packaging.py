...
def build_portable_package_plan(
    ...
    *,
    ...
    package_name = f"{packaged_at.strftime('%Y%m%d-%H%M%S')}-portable-{sanitize_artifact_name(name)}"
...
def materialize_portable_package(plan: PortablePackagePlan, repo_root: Path) -> dict[str, object]:
...
def uninstall_portable_package(target_dir: Path) -> PortableUninstallResult:
    ...
    entries = tuple(str(item) for item in entries_raw) if isinstance(entries_raw, list) else ()
    removed: list[str] = []
...
def build_distribution_package_plan(
    ...
    *,
    ...
    packaged_at = packaged_at or datetime.now(UTC)
    package_name = f"{packaged_at.strftime('%Y%m%d-%H%M%S')}-distribution-{sanitize_artifact_name(name)}"
    package_dir = workspace.exports / package_name
    archive_file = workspace.exports / f"{package_name}.zip"
...
def materialize_distribution_package(plan: DistributionPackagePlan, repo_root: Path) -> dict[str, object]:
    ...
    manifest: dict[str, object] = {
        ...
        "install_command": f"uv run protolink --install-portable-package {portable_dest.name} <target-dir>",
...
def build_installer_staging_plan(
    ...
    *,
    ...
    packaged_at = packaged_at or datetime.now(UTC)
    package_name = f"{packaged_at.strftime('%Y%m%d-%H%M%S')}-installer-{sanitize_artifact_name(name)}"
    package_dir = workspace.exports / package_name
    archive_file = workspace.exports / f"{package_name}.zip"
    ...
    install_ps1.write_text(
        "\n".join(
            (
                ...
                f"uv run protolink --install-distribution-package {distribution_dest.name} .\\staging $TargetDir",
            )
        )
        + "\n",
        encoding="utf-8",
...
def build_installer_package_plan(
    ...
    *,
    ...
    packaged_at = packaged_at or datetime.now(UTC)
    package_name = f"{packaged_at.strftime('%Y%m%d-%H%M%S')}-installer-package-{sanitize_artifact_name(name)}"
    package_dir = workspace.exports / package_name
    archive_file = workspace.exports / f"{package_name}.zip"
...
def materialize_installer_package(plan: InstallerPackagePlan, repo_root: Path) -> dict[str, object]:
    ...
    install_ps1.write_text(
        "\n".join(
            (
                "param([string]$TargetDir = '.\\installed')",
                f"uv run protolink --install-installer-staging {installer_staging_dest.name} .\\staging $TargetDir",
            )
        )
        + "\n",
        encoding="utf-8",
